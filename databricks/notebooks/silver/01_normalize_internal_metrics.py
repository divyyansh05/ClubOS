...
METRIC_DICT_PATH = "/Volumes/clubos/main/seeds/metric_dictionary.json"
...
def normalize_internal_asset(table_name, asset_name, asset_type):
    df = spark.read.table(f"clubos_bronze.{table_name}")
    df = clean_columns(df)
...
# Write wide tables
main_web.write.format("delta").mode("overwrite").saveAsTable("clubos_silver.silver_internal_main_website")
ecomm.write.format("delta").mode("overwrite").saveAsTable("clubos_silver.silver_internal_ecommerce")
stream.write.format("delta").mode("overwrite").saveAsTable("clubos_silver.silver_internal_streaming")
fan_app.write.format("delta").mode("overwrite").saveAsTable("clubos_silver.silver_internal_fan_app")
...
fact_all.write.format("delta").mode("overwrite").saveAsTable("clubos_silver.silver_internal_asset_metrics")
