...
def run_benchmark_coverage_check(severity: str = "REQUIRED") -> None:
    table_name = "clubos_silver.silver_benchmark_asset_metrics"
    df = spark.read.table(table_name)
...
def run_month_coverage_check(severity: str = "REQUIRED") -> None:
    table_name = "clubos_silver.silver_internal_asset_metrics"
    df = spark.read.table(table_name)
...
# Required checks
run_condition_check("clubos_silver.silver_internal_asset_metrics", "No null months", "month IS NOT NULL")
run_condition_check("clubos_silver.silver_internal_asset_metrics", "No null metric values", "metric_value IS NOT NULL")
run_duplicate_check("clubos_silver.silver_internal_asset_metrics", ["month", "asset_name", "metric_name"])
run_duplicate_check("clubos_silver.silver_benchmark_asset_metrics", ["month", "asset_name", "metric_name", "club"])
...
log_df.write.format("delta").mode("append").saveAsTable("clubos_silver.silver_data_quality_checks")
