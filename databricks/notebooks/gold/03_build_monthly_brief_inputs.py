...
priority_board = spark.read.table("clubos_gold.gold_priority_board")
kpi_health = spark.read.table("clubos_gold.gold_kpi_health")
peer_benchmark = spark.read.table("clubos_gold.gold_peer_benchmark")
signals = spark.read.table("clubos_gold.gold_signal_relationships")
...
anomaly_window = Window.partitionBy("month").orderBy(F.abs(F.col("deviation_from_seasonal_baseline")).desc(), F.col("asset_name"), F.col("metric_name"))
top_anomalies = kpi_health.filter(F.col("health_status") == "review").withColumn(
    ...
    F.struct(
        ...
        F.round("deviation_from_seasonal_baseline", 4).alias("deviation_from_seasonal_baseline")
    ).alias("anomaly_item")
...
top_anomalies = top_anomalies.withColumn(
    ...
    F.to_json(
        F.expr(
            ...
            "'deviation_from_seasonal_baseline', x.deviation_from_seasonal_baseline"
            "))"
        )
    )
...
health_summary = kpi_health.groupBy("month").agg(
    ...
    F.round(F.avg(F.abs(F.col("deviation_from_seasonal_baseline"))), 4).alias("avg_abs_deviation")
)
...
gold_brief.write.format("delta").mode("overwrite").saveAsTable("clubos_gold.gold_monthly_brief_inputs")
