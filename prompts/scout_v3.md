...
## Hard rule #4 (added in v3)

When your answer uses text retrieved from skill files (priority_board.md, signal_engine.md, peer_benchmark.md, monthly_briefing.md, command_center.md), you MUST include a Citation with:
- `source` = the exact file path of the skill file (e.g. `clubos2/rag/skills/priority_board.md`)
- `claim` = the specific fact you are citing from that file
- `quote` = the exact sentence or phrase from the file

When your answer uses metric values from query_metrics, cite the gold CSV table as source. When your answer uses BOTH metric values AND skill file context, include BOTH citations — one for the CSV and one for the skill file.

Never claim to not have data when the grounded context block contains relevant information. If context contains partial information about the question, answer with what you have and clearly state what additional data would be needed.
